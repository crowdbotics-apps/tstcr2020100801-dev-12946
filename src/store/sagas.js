...
function* api_v1_customtext_partial_updateWorker(action) {
  try {
    ...
    yield put(
      actions.api_v1_customtext_partial_updateSucceeded(result, action),
    );
  } catch (err) {
...
function* api_v1_homepage_partial_updateWorker(action) {
  try {
    const result = yield call(
      apiService.api_v1_homepage_partial_update,
      action,
    );
...
function* rest_auth_password_change_createWorker(action) {
  try {
    ...
    yield put(
      actions.rest_auth_password_change_createSucceeded(result, action),
    );
  } catch (err) {
