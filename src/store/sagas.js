...
function* registration_post_restauth_login_createWorker(action) {
  try {
    const result = yield call(
      apiService.registration_post_restauth_login_create,
      action
    )
    yield put(
      actions.registration_post_restauth_login_createSucceeded(result, action)
    )
  } catch (err) {
    yield put(
      actions.registration_post_restauth_login_createFailed(err, action)
    )
  }
}
function* registration_post_restauth_login_createWatcher() {
  yield takeEvery(
    types.REGISTRATION_POST_RESTAUTH_LOGIN_CREATE,
    registration_post_restauth_login_createWorker
  )
}
...
export default function* rootSaga() {
  const sagas = [
    registration_post_restauth_login_createWatcher,
